"""Train MAPPO on CityLearn v3 using the official HARL backend.

MAPPO (Multi-Agent PPO) is the homogeneous baseline for HAPPO: all 17 agents
share a single policy network (share_param=True) and update in fixed order
(fixed_order=True).  This allows direct comparison of heterogeneous-agent
(HAPPO) vs. homogeneous-agent (MAPPO) policy optimisation on the same task.
"""
...
def parse_args():
    ...
    parser.add_argument("--exp-name", default="citylearn_v3_mappo")
...
def main() -> int:
    ...
    output_dir = resolve_output_dir(args.output_dir, "mappo", args.scenario, args.seed)
    ...
    # MAPPO uses the same YAML defaults as HAPPO but overrides share_param and fixed_order
    algo_args, env_args = get_defaults_yaml_args("mappo", "gym")
    ...
    # MAPPO: homogeneous — all agents share one policy; updates follow a fixed order
    algo_args["algo"]["share_param"] = True
    algo_args["algo"]["fixed_order"] = True
    ...
    runner_args = {"algo": "mappo", "env": "gym", "exp_name": args.exp_name}
    runner = RUNNER_REGISTRY["mappo"](runner_args, algo_args, env_args)
    ...
    gradient_guard = install_harl_finite_optimizer_step_guard(
        runner,
        output_dir / "data" / "mappo_finite_gradient_guard.jsonl",
    )
    reward_metadata = {}
    ...
    hyperparameters = {
        ...
        "algorithm_family": "MADRL_BASELINE",
        "reward_function": "CityLearnV3MADRLRewardFunction",
        "reward_profile": "MAPPO",
    ...
    try:
        heartbeat_stop, heartbeat_thread = start_live_progress_heartbeat(
            ...
            active_stage="mappo_backend_training",
            initial_stage="mappo_backend_starting",
            note="MAPPO backend is collecting rollouts or updating the shared policy network.",
        )
        runner.run()
        if heartbeat_adapter is not None:
            try:
                heartbeat_adapter.write_live_heartbeat(
                    stage="mappo_backend_finished",
                    note="MAPPO backend runner finished; writing final artifacts.",
                )
            except Exception:
                pass
        runner.save()
